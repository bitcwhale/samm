...
for method in methods:
    ...
    for year in range(start_year, end_year + 1):
        ...
        mask_lb    = (simple_returns.index >= window_start) & (simple_returns.index <= window_end) window_lb  = simple_returns.loc[mask_lb] valid_w    = window_lb.notna().sum()                        # count non-NA per asset eligible_assets = valid_w[valid_w >= 60].index.tolist()     # need ≥60 months
