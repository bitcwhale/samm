...
# 1) Filter ESG data for AMER firms
scope1_us = scope1_df[scope1_df["Firm_Name"].isin(amer_names)]
scope2_us = scope2_df[scope2_df["Firm_Name"].isin(amer_names)]

# 2) Make every column name a string (so 2013 → "2013", etc.)
scope1_us.columns = scope1_us.columns.map(str)
scope2_us.columns = scope2_us.columns.map(str)

# 3) Pick out only the year columns
year_cols = [col for col in scope1_us.columns if col.isdigit()]

# 4) Pivot into wide tables for quick look-ups
scope1_wide = scope1_us.set_index('Firm_Name')[year_cols].astype(float)
scope2_wide = scope2_us.set_index('Firm_Name')[year_cols].astype(float)

# 5) ESG-coverage test: ≥7 years total, ≥5 consecutive
def check_scope(row):
    # 1) Build a zero/one array of valid years
    vals = row.notna().astype(int).values

    # 2) Total count
    total_years = vals.sum()

    # 3) Compute longest consecutive 1-run
    max_consec = 0
    cur = 0
    for v in vals:
        if v == 1:
    ...
    # 4) Return True only if >=7 total and >=5 consecutive
    return (total_years >= 7) and (max_consec >= 5)
...
simple_returns = np.log(prices / prices.shift(1))
simple_returns = simple_returns.dropna(how="all")  # Drop rows with NaN returns
...
def compute_mvp_weights(
    ...
    Assumes that `returns_window` has already been filtered for:
      1) ≥60 valid months in lookback
      2) 12 valid months in forecast year
      3) carbon data availability in prior year
    """

    # Align on the same set of assets
    assets = list(returns_window.columns)
    if len(assets) < 2:
        return pd.Series(np.nan, index=assets)

    # Re-index the other inputs to the same assets & time
    mkt_caps_window = mkt_caps_window[assets].reindex(returns_window.index, method='ffill')
    revenues_window = revenues_window[assets].reindex(returns_window.index, method='ffill')
    ...
    # ---------------------------------------------------
    if method == 'lw':
        # fill missing returns with zero for estimation
        R = returns_window.fillna(0).values
        lw = LedoitWolf()
        lw.fit(R)
        cov_matrix = lw.covariance_

    elif method == 'pinv':
        cov_matrix = np.cov(returns_window.fillna(0).values, rowvar=False)
        cov_pinv   = np.linalg.pinv(cov_matrix)
        ones       = np.ones(len(assets))
        w          = cov_pinv @ ones / (ones.T @ cov_pinv @ ones)
        return pd.Series(w, index=assets)

    elif method in ('factor_1', 'factor_3'):
        # your existing factor-based logic here
        factors = construct_factors(returns_window, mkt_caps_window, revenues_window, rf_window)
        if method == 'factor_1':
            factors = factors[['Mkt-RF']]
        cov_matrix, companies = compute_factor_cov_matrix(returns_window, factors)
        if not companies:
            return pd.Series(np.nan, index=assets)
        # restrict to surviving companies
        assets = companies
    elif method == 'identity':
        sample_cov = np.cov(returns_window.fillna(0).values, rowvar=False)
        avg_var    = np.trace(sample_cov) / len(assets)
        cov_matrix = (0.8 * sample_cov) + (0.2 * np.eye(len(assets)) * avg_var)
    else:
        raise ValueError("Method must be 'lw', 'pinv', 'factor_1', 'factor_3', or 'identity'.")

    # ---------------------------------------------------
    # 2) Build and solve the quadratic program
    # ---------------------------------------------------
    n = len(assets)
    w = cp.Variable(n)
    constraints = [
    ...
    if turnover_limit is not None and prev_weights is not None:
        prev = prev_weights.reindex(assets).fillna(0).values
        constraints.append(cp.norm1(w - prev) <= turnover_limit)

    if carbon_constraint is not None:
        c_vec, threshold = carbon_constraint
        c_aligned = c_vec.reindex(assets).fillna(0).values
        constraints.append(w @ c_aligned <= threshold)

    objective = cp.Minimize(cp.quad_form(w, cp.psd_wrap(cov_matrix)))
    if transaction_cost is not None and prev_weights is not None:
        prev = prev_weights.reindex(assets).fillna(0).values
        cost = transaction_cost * cp.norm1(w - prev)
        objective = cp.Minimize(cp.quad_form(w, cp.psd_wrap(cov_matrix)) + cost)
...
# ---------------------------------
rolling_window_months = 120   # 10 yrs × 12 mos
min_required_months  = 60     # at least 60 non‐NA months
...
for method in methods:
    ...
    for year in range(start_year + 1, end_year + 1):
        # 1) Define lookback & forecast windows
        ...
        # 2) Who has ≥60 valid months in lookback?
        mask_lb = (simple_returns.index >= window_start) & (simple_returns.index <= window_end)
        window = simple_returns.loc[mask_lb]
        valid_w = window.notna().sum()
        assets_w = set(valid_w[valid_w >= min_required_months].index)

        # 3) Who has all 12 months in the forecast year?
        mask_f = (simple_returns.index >= forecast_start) & (simple_returns.index <= forecast_end)
        year_data = simple_returns.loc[mask_f]
        valid_y = year_data.notna().sum()
        assets_y = set(valid_y[valid_y == 12].index)

        # 4) Who has carbon data in year‐1?
        prev = str(year - 1)
        if prev in scope1_wide.columns and prev in scope2_wide.columns:
            c1 = set(scope1_wide[scope1_wide[prev].notna()].index)
            c2 = set(scope2_wide[scope2_wide[prev].notna()].index)
            assets_c = c1 & c2
        else:
            assets_c = set()

        # 5) Final eligible set
        eligible = list(assets_w & assets_y & assets_c)
        print(f"Year {year}: lookback={len(assets_w)}, forecast={len(assets_y)}, carbon={len(assets_c)} → eligible={len(eligible)}")

        if not eligible:
            mvp_weights[year] = pd.Series(dtype=float)
            continue

        # 6) Slice inputs by boolean mask + eligible list
        returns_window  = simple_returns.loc[mask_lb, eligible]
        mask_mkt        = (mkt_caps.index >= window_start) & (mkt_caps.index <= window_end)
        mkt_caps_window = mkt_caps.loc[mask_mkt, eligible]
        mask_rev        = (revenues.index >= window_start) & (revenues.index <= window_end)
        revenues_window = revenues.loc[mask_rev, eligible]
        mask_rf         = (rf_series.index >= window_start) & (rf_series.index <= window_end)
        rf_window       = rf_series.loc[mask_rf]

        # 7) Compute MVP weights
        wts = compute_mvp_weights(
            returns_window,
            mkt_caps_window,
            revenues_window,
            rf_window,
...
for Y in range(2013, 2025):  # From 2013 to 2025
    ...
    common_firms = weights_Y_minus_1.index.intersection(emissions_Y.index).intersection(mkt_cap_Y.index).intersection(
        revenue_Y.index)
...
plt.plot(list(carbon_footprints.keys()), list(carbon_footprints.values()), marker='o',
         label='Carbon Footprint (tons CO2e per million USD invested)')
plt.plot(list(waci_values.keys()), list(waci_values.values()), marker='s',
         label='WACI (tons CO2e per million USD of revenue)')
...
end_year = 2024  # Adjust based on your data availability
tracking_weights = {}
method = 'lw'
